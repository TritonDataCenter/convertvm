...
VMDK.prototype.open = function (callback) {
    ...
    fs.stat(self.options.filename, function (stat$error, stats) {
        self.fileSize = stats.size;
        console.warn(self.fileSize);

        fs.open(self.options.filename, 'r', function (open$error, fd) {
            if (open$error) {
                return callback(open$error);
            }
            self.fd = fd;
            self.parser.typedef('SectorType', 'uint32_t[2]');
            return self.parseHeader(callback);
        });
    });
};
...
VMDK.prototype.directory = function (callback) {
    ...
    self.footer(function (footer$error, footer) {
        var gdOffset = footer.gdOffset[0]*512;
        console.warn('grain directory offset was ' + gdOffset);
        self.markerAt(gdOffset-512, function (marker$error, marker) {
            console.warn('Grain directory marker');
            console.warn(util.inspect(marker));

            self.dataAt(gdOffset, 512, function (data$error, buffer) {
                var directory = self.parser.readData(
                    GrainDirectoryStruct, buffer, 0);
...
VMDKStream.prototype.start = function () {
     ...
     * set in the marker's 'size' field. Each time we do this, we will
     * decompress the data and then emit it as a 'data' event.
     *
     ...
     *
     * Because Node doesn't let us seek on a file descriptor directly, we'll
     * keep a running offset within the file.
     */
            ...
            self.vmdk.getMarker(self.offset, function (error, type, marker) {
                ...
                if (type === 'gt') {
                    ...
                    return fs.read(
                        ...
                        function (read$error, bytesRead, buf) {
                            var table = self.vmdk.parser.readData(
                                GrainTableStruct, gtBuf, 0);
    ...
    async.forEachSeries(
        ...
        function (offset, fe$callback) {
            ...
            if (offset === 0) {
                ...
                offset = offset * 512;
                return self.vmdk.markerAt(offset, function (error, marker) {
                    self.parseCompressedGrainData(
                        offset + 12, marker.size, fe$callback);
                });
            }
        },
        function (error) {
...
function (grainOffset, markerSize, callback) {
    ...
    async.whilst(
        ...
        function (whilst$callback) {
            var readSize
                = toRead > DEFAULT_READ_SIZE ? DEFAULT_READ_SIZE : toRead;
            ...
            fs.read(
                ...
                function (error, bytesRead, buf) {
                    ...
                    return (
                        gunzip.write(
                            buf,
                            function (gunzip$error, decompressed) {
                                if (error) {
                                    console.error(
                                        'gz error ' + gunzip$error.message);
                                    throw gunzip$error;
                                    // xxx handle gz error
                                }
        ...
        function (error) {
            ...
            return gunzip.close(function (gunzip$error, decompressed) {
                if (error) {
                    return console.error('gz error ' + gunzip$error.message);
                }
                if (decompressed) {
                    self.emit('data', decompressed);
                }
